...
export function TableRow({ data, mode, clickable, ...props }: TableRowProps) {
  ...
  return (
    <tr className={TableRowVariants({ mode, clickable })} {...props}>
      {data.map((d) => d === 'presente' || d === 'ausente' ? 
        <td><Button onClick={presenceButtonTap} className={presenceText ? 'presence-check-button' : 'abscense-button'}>{presenceText ? 'presente' : 'ausente'}</Button></td> : 
        <td>{d}</td>
      )}
