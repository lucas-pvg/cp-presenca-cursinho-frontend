...
const createLessonVariants = cva(
...
export function CreateLesson({ mode, variant, close, className }: createLessonProps) {
  ...
  const handleClose = () => {
    close()
    setLessonData(
      { name: '', subject: 'default', studentClass: 'default', date: '', startTime: '', endTime: '' }
    )
  }
          ...
          <div className='content-body'>
            <form id='class-form' onSubmit={handleSubmit}>
              <ModalRow labels={['Nome do evento']} mode={mode} >
                <Input type='text' name='name' value={lessonData.name} placeholder='Aula de Matemática' mode={mode} onChange={handleChange} />
              </ModalRow>

              <ModalRow labels={['Disciplina', 'Turma']} mode={mode}>
                <SelectInput placeholder='-- Disciplina --' name='subject' value={lessonData.subject} onChange={handleChange}>
                  {
                    subjects.map((subject) => <option key={subject.id} value={subject.name}>{subject.name}</option>)
                  }
                </SelectInput>

                <SelectInput placeholder='-- Turma --' name='studentClass' value={lessonData.studentClass} onChange={handleChange}>
                  {
                    studentClasses.map((studentClass) => <option key={studentClass.id} value={studentClass.name}>{studentClass.name}</option>)
                  }
                </SelectInput>
              </ModalRow>

              <ModalRow labels={['Data', 'Horário']} mode={mode}>
                <Input type='date' name='date' value={lessonData.date} mode={mode} onChange={handleChange} />
                <Input type='time' names={['startTime', 'endTime']} values={[lessonData.startTime, lessonData.endTime]} mode={mode} onChange={handleChange} />
              </ModalRow>
            </form>
          </div>
