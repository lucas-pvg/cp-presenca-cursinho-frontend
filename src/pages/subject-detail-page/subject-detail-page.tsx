...
export function SubjectDetailPage({ mode, ...props }: SubjectDetailPageProps) {
  ...
  const [lessonDatetimes, setLessonDatetimes] =
    useState<LessonRecurrentDatetime[]>();
  const getDatetimes = () => {
    Services.listRecurrencyFromParams(
      subjects[subjectIndex].id,
      studentClasses[classIndex].id
    )
      .then((data) => {
        setRecurrency(data);
  ...
  const handleChange = (e: any) => {
    ...
    setLessonDatetimes((prevData) => {
      ...
      if (prevData) {
        data = [...prevData];
        data[id] = new LessonRecurrentDatetime({
          ...prevData[id],
          [name]: value,
        });
            ...
            {subjects.map((sub) => {
              return (
                <Card
                  ...
                  key={sub.id}
                  variant={
                    subjects[subjectIndex].id == sub.id
                      ? 'primary'
                      : 'secondary'
                  }
                  ...
                  return (
                    <div className="schedule-row" key={datetime.id}>
                      ...
                      <Icon
                        ...
                        iconType="x-circle"
                        size={32}
                        onClick={() => deleteDatetime(datetime.id)}
                      />
                    </div>
                  );
                })}
              </div>
              <Button variant="outline" onClick={() => createDatetime()}>
                Nova Data
              </Button>
            </div>
          </div>
